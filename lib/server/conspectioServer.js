module.exports = (http) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
      ...
      if(currNode.origin) { // this is a VIEWER
        ...
        // update the sourceNode's leechers
        ...
        function recurseChangeSource(node, targetOrigin, targetSource) {
          // iterate through node's leechers
          for(var i = 0; i < node.leechers.length; ++i) {
            var currLeecher = alasql('SELECT * FROM nodeTracker WHERE socketId = ? AND origin = ?', [node.leechers[i], targetOrigin])[0];

            if(targetSource) { // first change source
              // update currLeecher's source to be targetSource
              currLeecher.source = targetSource;
              alasql('UPDATE nodeTracker SET source = ? WHERE socketId = ? AND origin = ?', [targetSource, currLeecher.socketId, currLeecher.origin]);

              // emit msg to targetSource to 'initiateConnection' with currLeecher
        ...
        recurseNodes(currNode, 'b1Id', function(removeId, origin) {
          
          console.log('inside doFunc: removeId, origin', removeId, origin);
          
          alasql('DELETE from nodeTracker WHERE socketId = ? AND origin = ?', [removeId, origin]);

          // client side notification - emit message to leechers that broadcaster left to clean up conspectio.connections{}
          if(origin) {
            io.to(removeId).emit('broadcasterLeft', origin);
          }        
        });  
      }
  ...
  function recurseNodes(node, targetOrigin, doFunc) {
    console.log('inside recurseNodes', node);
    ...
    doFunc(node.socketId, node.origin); // if not a broadcaster, pass in targetOrigin?
  }

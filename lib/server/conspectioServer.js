module.exports = (http) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('initiateView', (eventId) => {
      ...
      var broadcastersList = alasql('SELECT * FROM nodeTracker WHERE eventId=? AND origin="" LIMIT 2', [eventId]); 
      ...
      console.log('broadcastersList:', broadcastersList);
      broadcastersList.forEach((broadcaster) => {
        console.log('broadcaster inside for each:', broadcaster);
        var sourcesToCheck = [broadcaster.socketId];
        var foundCapacity = false;
        
        // check the degree where sourcesToCheck is at?
          // var degreeLevel = alasql('SELECT degree FROM nodeTracker WHERE socketId = ? AND origin = ?', [sourcesToCheck[0], broadcaster.socketId])[0].degree;

        var originId = '';

        console.log('broadcaster socket id:', broadcaster.socketId);
        while(!foundCapacity){
          ...
          if(broadcasterNode) {
            ...
            // console.log('broken:',alasql('SELECT degree FROM nodeTracker WHERE socketId = ? AND origin = ?', [sourcesToCheck[0], broadcaster.socketId]));
            // check the degree where sourcesToCheck is at?
            // var degreeLevel = alasql('SELECT degree FROM nodeTracker WHERE socketId = ? AND origin = ?', [sourcesToCheck[0], originId])[0].degree;

            // originId = (degreeLevel === 0) ? '': broadcaster.socketId;
            ...
            originId = broadcaster.socketId; // post first
          }
        } //end while loop
      });//end of forEach
    });
